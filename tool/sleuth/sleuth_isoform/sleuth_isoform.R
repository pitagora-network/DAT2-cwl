...
write.table(kallisto.df, "output_isoform/kallisto_res.txt", row.names=F, quote=F)
...
# 結果を保存する
write.table(LRT_table, "output_isoform/LRT_res.sorted.txt", row.names=F, quote=F, sep="\t")
...
# 図を保存する
ggplot2::ggsave("output_isoform/ENST00000503567.5.png", p)
ggplot2::ggsave("output_isoform/ENST00000503567.5.pdf", p)
...
pdf('output_isoform/heatmap.pdf')
plot_transcript_heatmap(LRT, transcripts, units="tpm")
dev.off()
...
write.table(WT_table, "output_isoform/WT_res.sorted.txt", row.names=F, quote=F, sep="¥t")

#Wald検定の結果をvolcano plotで図示する。デフォルト設定ではq値が0.1を下回る転写産物が赤いプロットで示される。
p3 <- plot_volcano(WT, 'conditionType_1_Diabetes', 'wt')
ggsave("output_isoform/volcanoplot.png", p3)
