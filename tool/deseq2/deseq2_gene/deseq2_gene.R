...
# データフレームの保存
write.table(res_wt_naomit_annot_sort, "output_gene/Wald-test.result.txt", sep="\t", quote=F, row.names=F)

# 作図して表示させるコマンド
plotMA(res_wt, ylim=c(-2, 2))

# 保存するには下記コマンド
png("output_gene/MAplot.png") # EPS形式にしたい場合はpostscript("MAplot.eps")を使う
plotMA(res_wt, ylim=c(-2, 2))
dev.off()
...
# 保存する際はMA-plotと同様に
png("output_gene/ENSG00000169710.png")
plotCounts(dds, gene="ENSG00000169710", intgroup="condition")
dev.off()
